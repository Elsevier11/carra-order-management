...
import { z } from 'zod'
import { responsabiliInterni } from '../../db/schema'
import { db } from '../db'
...
router.get('/', requireAuth, async (_req, res, next) => {
  try {
    const rows = await db.select().from(responsabiliInterni).orderBy(asc(responsabiliInterni.nome))
...
router.delete('/:id', requireAuth, requireRole(['admin']), async (_req, res, next) => {
  try {
    ...
    if (!Number.isFinite(id)) return res.status(400).json({ message: 'Invalid id' })
    const [deleted] = await db.delete(responsabiliInterni).where(eq(responsabiliInterni.id, id)).returning({ id: responsabiliInterni.id })
    if (!deleted) return res.status(404).json({ message: 'Responsabile non trovato' })
